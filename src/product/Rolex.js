...
function Rolex({getdata}) {
 ...
 const [count, setcount] = useState({});
  ...
  async function getall() {
    ...
    let data = await getalltypeproduct({ type: "ROLEX", page: page });
...
const handlePageChange = async (id) => {
    setpage(id)
     let panigation = await getalltypeproduct({ type: "ROLEX", page: id });
      if (panigation) {
      ...
      <Banner span={ "ROLEX"}/>
        ...
        <div className="product-container-right">
          ...
          <div className="product-container-right-title">
            <p>Hiển thị <span style={{color:"orange"}}>1</span>-<span style={{color:"orange"}}>{count.rolex}</span> của <span style={{color:"orange"}}>{count.rolex}</span> kết quả</p>
