...
async function getall() {
  let data = await getalltypeproductbrand({ type: "ĐỒNG HỒ DÂY DA",page:page });
...
const handlehead = async(id,name,price,image) => {
  ...
  const handlePageChange = async (id) => {
    setpage(id)
     let panigation = await getalltypeproductbrand({ type: "ĐỒNG HỒ DÂY DA",page:id });
      if (panigation) {
  ...
  return (
    <div>
      {loading ? <Loading check={loading} /> :
        <div className='donghodayda-container'> 
          <Header getdata={getdata}/>
          <Banner span={ "ĐỒNG HỒ DÂY DA"} />
