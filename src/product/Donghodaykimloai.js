...
import { getalltypeproduct, getalltypeproductbrand, getallwhitlist, getcountproduct, getstar, whitlist } from '../servide/service';
...
function Donghodaykimloai({getdata}) {
...
async function getall() {
  let data = await getalltypeproductbrand({ type: "ĐỒNG HỒ DÂY KIM LOẠI",page:page });
  ...
  useEffect(() => {
   ...
   const handlechange = async(e) => {
    ...
    if (value === "1") {
      ...
      let data = await getalltypeproductbrand({
        type: "ĐỒNG HỒ DÂY KIM LOẠI",page:page,
          check:   "reduce" 
      });
      console.log(data);
     let res = await getstar();
...
const handlehead = async(id,name,price,image) => {
  ...
  const handlePageChange = async (id) => {
    setpage(id)
    let panigation = await getalltypeproductbrand({ type: "ĐỒNG HỒ DÂY KIM LOẠI", page: id });
  ...
  return (
    <div>
      {loading ? <Loading check={loading} /> :
        <div className='donghodaykimlai-container'>
          <Header getdata={getdata}/>
          <Banner span={ "ĐỒNG HỒ DÂY KIM LOẠI"} />
          <div className='product-container'>
            ...
            <div className="product-container-right">
              ...
              <div className="product-container-right-title">
                <p>Hiển thị <span style={{color:"orange"}}>1</span>-<span style={{color:"orange"}}>{count.donghodayda}</span> của <span style={{color:"orange"}}>{count.donghodayda}</span> kết quả</p>
...
export default Donghodaykimloai;